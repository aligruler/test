 ...
 * @version 13810
 */
public class MultiStartSPupdateEnd {
	...
	private Solution bestSolCurrentPeriod = null;
	...
	MultiStartSPupdateEnd(Test myTest, Inputs myInputs, Random myRng) {
		aTest = myTest;
		inputs = myInputs;
		rng = myRng;
	}
	...
	public Outputs solve() {
		...
		for (int i = 0 ; i < aTest.getNumPeriods(); i++){
			bestSolPerPeriod[i] = new Solution (initSolPerPeriod[i]);
		}
		System.out.println("INITIAL SOL ALL PERIODS: " + initialSolAllPeriods.getAccTotCosts());
		...
		for (int p = 0; p < aTest.getNumPeriods(); p++) {
			System.err.println("NEXT PERIOD");
			long start = ElapsedTime.systemTime();
			inputs = (Inputs) deepClone(InputsInitVector[p]);
			auxInitInput = (Inputs) deepClone(InputsInitVector[p]); 
			
			double elapsed = 0.0;	
			firstime = true;
			Simulation Sim = new Simulation(aTest, inputs);
			Sim.SimulateDemandDet();
			...
			bestSolCurrentPeriod = new Solution(initSol.constructInitialSols(indexBestInitSol));
			
			int iteration = 0;
			//while (elapsed < aTest.getMaxTime()) {
			while (iteration < aTest.getSimRuns()) {
				...
				if (newSolCurrentPeriod.getTotalCosts() < bestSolCurrentPeriod.getTotalCosts()) {
					shakingNum = 0;
					baseSolCurrentPeriod = new Solution(newSolCurrentPeriod);
					bestSolCurrentPeriod = new Solution(newSolCurrentPeriod);
					auxInput = (Inputs) deepClone(inputs);
					//inputs = (Inputs) deepClone(InputsVector[p]);
					//bestSolPerPeriod[p] = new Solution(vnsPeriodSols[p]);										
					System.err.println("new bestSol Period " + p + ": " + bestSolCurrentPeriod.getTotalCosts());
					bestSolCurrentPeriod.setTime(elapsed);
					firstime = false;
				} 
				else {
			...
			vnsPeriodSols[p] = new Solution(bestSolCurrentPeriod);
		...
		double totStock = 0.0;
		double totRouting = 0.0;
		for (int i = 0; i < aTest.getNumPeriods(); i++){
			totStock += vnsPeriodSols[i].getStockCosts();
			totRouting += vnsPeriodSols[i].getRoutingCosts();
		}
		bestSolAllPeriods.setAccStockCosts(totStock);
		bestSolAllPeriods.setAccRoutingCosts(totRouting);
		bestSolAllPeriods.setAccTotCosts(totStock + totRouting);
		bestSolAllPeriods.setDetCosts(totStock + totRouting);
		
		
		//////==================Sim
		for(int tper = 0; tper <vnsPeriodSols.length;tper++){
			Simulation Sim = new Simulation(aTest, InputsVector[tper]);
			Sim.SimulateDemandSto();
			updateCurrentLevel();		
		}

		
        ///Calculo stockCost
		 float TotStockCost = 0;
		 for (int i = 0; i < aTest.getNumPeriods(); i++){
			 float StCostPeriod = 0;
			 for( int j = 1; j < InputsVector[i].getNodes().length; j++ )
			 {   
					Node aNode = InputsVector[i].getNodes()[j];   
        	        StCostPeriod +=  aNode.getActivePolicy().getExpStockCosts();
        	        //System.out.println("node: " + j + " "+ "StCostPeriod: " + StCostPeriod);
			 }
			 vnsPeriodSols[i].setStockCosts(StCostPeriod);
			 vnsPeriodSols[i].setTotalCosts(StCostPeriod + vnsPeriodSols[i].getRoutingCosts());
			 TotStockCost += StCostPeriod;
		}        

		bestSolAllPeriods.setAccStockCosts(TotStockCost);
		bestSolAllPeriods.setAccRoutingCosts(totRouting);
		bestSolAllPeriods.setAccTotCosts(TotStockCost + totRouting);
			
		System.out.println("BEST SOLUTION IS FOUND: " + bestSolAllPeriods.getAccTotCosts());
		
		outList.add(bestSolAllPeriods);
		outputs.setBestPerSol(vnsPeriodSols);
